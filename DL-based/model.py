...
class FeatureExtraction(torch.nn.Module):
    def __init__(
        ...
        if feature_extraction_cnn == 'resnet101':
            ...
            if last_layer == '':
                last_layer = 'layer3'
            resnet_module_list = [getattr(self.model, l)
                                  for l in resnet_feature_layers]
        ...
        if feature_extraction_cnn == 'resnet101fpn':
            if feature_extraction_model_file != '':
                ...
                resnet = models.resnet101(weights=models.ResNet101_Weights.DEFAULT)
            resnet_module_list = [getattr(resnet, l)
                                  for l in resnet_feature_layers]
...
class NeighConsensus(torch.nn.Module):
    ...
    def forward(self, x):
        if self.symmetric_mode:
            ...
            # this second result is "transposed back" to the A-B ordering to
            # match the first result and be able to add together
...
def maxpool4d(corr4d_hres, k_size=4):
        ...
        for j in range(k_size):
            for k in range(k_size):
                for l in range(k_size):
                    slices.append(
                        corr4d_hres[:, 0, i::k_size, j::k_size, k::k_size, l::k_size].unsqueeze(0))
    ...
    max_j = torch.fmod(max_idx.sub(max_l).div(
        k_size).sub(max_k).div(k_size), k_size)
    max_i = max_idx.sub(max_l).div(k_size).sub(
        max_k).div(k_size).sub(max_j).div(k_size)
    # i,j,k,l represent the *relative* coords of the max point in the box of
    # size k_size*k_size*k_size*k_size
...
class ImMatchNet(nn.Module):
    def __init__(self,
        ...
        if checkpoint is not None and checkpoint is not '':
            ...
            for name, param in self.Classifier1.state_dict().items():
                self.Classifier1.state_dict()[name].copy_(
                    checkpoint['state_dict']['Classifier1.' + name])
            print('Done!')
    ...
    # used only for foward pass at eval and for training with strong
    # supervision
    def forward(self, tnf_batch):
        ...
        def to_cuda(x): return x.cuda() if corr4d.is_cuda else x
        batch_size, ch, fs1, fs2, fs3, fs4 = corr4d.size()
        softarg2d = SoftArgmax2D()

        # PROJECTING ON B FOR EACH PIXEL IN A
        nc_B_Avec = corr4d.view(batch_size, fs1 *
                                fs2, fs3, fs4)  # [batch_idx,k_A,i_B,j_B]

        nc_B_Avec = torch.nn.functional.softmax(nc_B_Avec, dim=1)

        a1 = softarg2d(nc_B_Avec)

        A_vec = feature_A.view(feature_A.size(0), feature_A.size(
            1), -1).type(torch.cuda.FloatTensor)
        a2 = torch.zeros(
            A_vec.size(0),
            A_vec.size(2)).type(
        ...
        for i in range(A_vec.size(2)):
            t = feature_B[:, :, a1[0, i, 1].type(torch.cuda.ByteTensor).item(
            ), a1[0, i, 0].type(torch.cuda.ByteTensor).item()]
            u = A_vec[:, :, i]
        ...
        # .permute(0,3,1,2) # [batch_idx,k_A,i_B,j_B]
        ...
        B_vec = feature_B.view(feature_B.size(0), feature_B.size(
            1), -1).type(torch.cuda.FloatTensor)
        b2 = torch.zeros(
            B_vec.size(0),
            B_vec.size(2)).type(
        ...
        for i in range(B_vec.size(2)):
            t = feature_A[:, :, b1[0, i, 1].type(torch.cuda.ByteTensor).item(
            ), b1[0, i, 0].type(torch.cuda.ByteTensor).item()]
            u = B_vec[:, :, i]
            # sum(a * b) / sqrt(sum(a ^ 2) * sum(b ^ 2))
            multi = torch.sum(torch.mul(t, u), dim=1)
            sum1 = torch.sum(torch.pow(t, 2), dim=1)
